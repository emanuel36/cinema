...
public:
   ...
   string search(string search){
    ...
    void addSessao(string filme, Sessao sessao, int idS){
        /*for(auto elem: sessoes){
            if((elem.filme->nome == sessao.filme->nome) && (elem.filme->td == sessao.filme->td)){
                if(elem.hora == sessao.hora){
                    throw string ("erro | sessão já existe");
                }
            }
        }*/
        ...
        for(auto elem : sessoes){
            if(elem.sala->id == idS){
                for(auto ele : filmes){
                    if(ele.nome == filme){
                        if((elem.filme->duracao + elem.hora) >= sessao.hora)
                            throw string("erro | sala ocupada");
                    }
                }
            }
        }
    ...
    void addFilme(Filme filme){
        for(auto elem: filmes){
            if((elem.nome == filme.nome) && (elem.td == filme.td)){
                return throw string("erro | filme já existe");
            }
        }
